...
| Network Tools activity | On-demand diagnostics (Ping, DNS, Traceroute, Port Check, HTTP Check, TLS Inspector, TCP Throughput, Latency Trend, Subnet Calculator, Wake-on-LAN, NTP, ARP Table, Network Overview, LAN Discovery) to hosts/networks you specify | **Each tool keeps a history** of what you typed and the result it showed, up to 1,000 entries per tool, locally on device only, so you can bring a run back; delete entries from the tool's history list or from the Cache Manager. Never transmitted off-device |
...
**Network Tools.** The Network Tools require explicit one-time consent before first use. Every operation is initiated manually by you and is directed only at the hosts/addresses/networks you specify; the App makes no automated or background network requests. Results and any read of the local ARP neighbor cache are shown on-device and are never transmitted off-device. Each tool keeps a history of its runs — what you typed and the result exactly as it was shown (up to 1,000 entries per tool) — in the App's private storage on your device, so you can bring a result back later. A result can contain what the tool itself showed you, such as host names, addresses of devices on your network, or your public IP address. Nothing in it is sent anywhere; you can delete entries from each tool's history list or from the Cache Manager.
...
*Last updated: 2026-09-23 | AP-Scout Pro v6.0.55*
